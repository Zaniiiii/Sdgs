...
class OpenLibrary:
    ...
    def parse_result(self, content):
        ...
        if result["abstract"] =="":
            try:
                # Find the specific <div> with the given classes
                target_div = parsed.find('div', class_='col-md-7 col-sm-12 col-xs-12')

                # Find all <p> tags within this <div>
                p_tags = target_div.find_all('p')

                # Combine the content of the first three <p> tags within the specific <div>
                combined_p_content = ''.join([p.text for p in p_tags[:3]])
                result["abstract"] = self.remove_html_tags(combined_p_content)
                result["keywords"] = ""
            except (AttributeError, IndexError) as e:
                result["abstract"] = ""
                result["keywords"] = ""
    ...
    def save_to_json(self, df, file_name, folder='./storage/result/scrappingOplib'):
